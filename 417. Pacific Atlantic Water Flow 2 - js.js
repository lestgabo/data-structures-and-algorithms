...
const pacificAtlantic = (matrix) => {
    ...
    const dfs = (matrix, pastValue, i, j, ocean) => {
        ...
        if (i < 0 || i > matrix.length - 1 || j < 0 || j > matrix[0].length - 1) {
            return;
        } else if (pastValue >= matrix[i][j]) {
            // means: the water only goes to the next grid if
            // the current Value (matrix[i][j]) is greater or equal to pastValue
            // because we imagine from that spot, water trickles DOWN to lower or equal values
            // but we are doing the opposite, going through water's path
            return;
        } else if (ocean[i][j] === 1) {
            return;
        }
